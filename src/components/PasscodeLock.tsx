  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 px-3 sm:px-4 overflow-y-auto py-4 sm:py-6 select-none"
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-[340px] sm:max-w-md bg-gradient-to-b from-slate-900/95 via-slate-900 to-rose-950/80 border border-rose-500/40 rounded-3xl p-5 sm:p-8 shadow-2xl backdrop-blur-2xl text-center text-white my-auto z-10"
        >
          {/* Lock Icon */}
          <motion.div
            whileHover={{ scale: 1.1, rotate: 5 }}
            className="mx-auto w-14 h-14 sm:w-20 sm:h-20 rounded-full bg-gradient-to-tr from-rose-500 to-pink-500 p-0.5 shadow-lg shadow-rose-500/40 mb-3 sm:mb-6"
          >
            <div className="w-full h-full bg-slate-950 rounded-full flex items-center justify-center">
              <Lock className="w-7 h-7 sm:w-10 sm:h-10 text-pink-400 animate-pulse" />
            </div>
          </motion.div>
          ...
          {/* PIN Indicators */}
          <div className="flex justify-center gap-2.5 sm:gap-3 mb-5 sm:mb-8">
            {Array.from({ length: Math.max(4, settings.passcode.length) }).map((_, idx) => (
              <motion.div
                key={idx}
                animate={idx < pin.length ? { scale: [1, 1.25, 1] } : { scale: 1 }}
          ...
          {/* Keypad */}
          <div className="grid grid-cols-3 gap-2.5 sm:gap-3 mb-5 max-w-[270px] sm:max-w-xs mx-auto">
            {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((num) => (
              <motion.button
                ...
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white/5 hover:bg-rose-500/20 active:bg-rose-500/40 border border-white/10 text-xl font-semibold transition-colors flex items-center justify-center mx-auto shadow-md"
              >
                {num}
              </motion.button>
            ))}
            ...
            <motion.button
              ...
              className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white/5 hover:bg-red-500/20 text-xs font-bold text-rose-300 border border-white/10 flex items-center justify-center mx-auto transition-colors"
            >
              Sil
            </motion.button>
          </div>
